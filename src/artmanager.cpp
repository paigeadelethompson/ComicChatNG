...
ArtManager::ArtManager(QObject *parent)
    : QObject(parent)
{
}
...
bool ArtManager::scan()
{
    ...
    const QFileInfoList avbs = dir.entryInfoList({QStringLiteral("*.avb")}, QDir::Files, QDir::Name);
    for (const QFileInfo &fi : avbs)
        m_avatarNames.append(fi.completeBaseName());

    const QFileInfoList bgbs = dir.entryInfoList({QStringLiteral("*.bgb"), QStringLiteral("*.bmp")},
...
Avatar *ArtManager::avatar(const QString &name)
{
    ...
    const QString path = QDir(m_artDir).filePath(fileBase + QStringLiteral(".avb"));
...
QString ArtManager::resolveDefaultArtDir()
{
    ...
    const QStringList candidates = {
        ...
        QDir(QCoreApplication::applicationDirPath()).filePath(QStringLiteral("comicart")),
        QDir(QCoreApplication::applicationDirPath()).filePath(QStringLiteral("../v2.5-beta-1-modern/comicart")),
        QDir(QCoreApplication::applicationDirPath()).filePath(QStringLiteral("../../v2.5-beta-1-modern/comicart")),
    };

...
{
    writeLine(QStringLiteral("PRIVMSG %1 :\x01""ACTION %2\x01").arg(target, text));
}
...
void IrcClient::processLine(const QString &line)
{
    ...
    if (command == QLatin1String("PING")) {
        writeLine(QStringLiteral("PONG :%1").arg(trailing.isEmpty() ? (args.value(0)) : trailing));
        return;
    }
...
void IrcClient::handleNumeric(int code, const QString &, const QStringList &args, const QString &trailing)
{
    switch (code) {
    case 1: // RPL_WELCOME
        m_registered = true;

...
Pose::Pose(const PoseLayerOffsets &layers) : m_layers(layers) {}
...
QImage Pose::composited() const {
  ...
  QImage mask = m_mask.isNull() ? QImage()
                                : m_mask.convertToFormat(QImage::Format_ARGB32)
                                      .scaled(src.size(), Qt::IgnoreAspectRatio,
                                              Qt::FastTransformation);
  QImage aura = m_aura.isNull() ? QImage()
                                : m_aura.convertToFormat(QImage::Format_ARGB32)
                                      .scaled(src.size(), Qt::IgnoreAspectRatio,
                                              Qt::FastTransformation);
  ...
  for (int y = 0; y < src.height(); ++y) {
    ...
    const QRgb *a = haveAura
                        ? reinterpret_cast<const QRgb *>(aura.constScanLine(y))
                        : nullptr;
...
Avatar::Avatar(AvatarKind kind) : m_kind(kind) {}
...
bool Avatar::handleCommonTag(AvbStream *stream, quint16 tag, quint16 size,
                             qint32 &adj) {
  switch (tag) {
  case AK_NAME: {
    QByteArray s;
...
bool Avatar::loadBodyRecs(AvbStream *stream, bool oldTag, qint32 &adj) {
  ...
  const quint32 recSize =
      oldTag ? sizeof(AvbBodyDataOld) : sizeof(AvbBodyDataNew);
  quint32 prevImage = 0;
...
bool Avatar::loadFaceRecs(AvbStream *stream, bool oldTag, qint32 &adj) {
  ...
  const quint32 recSize =
      oldTag ? sizeof(AvbFaceDataOld) : sizeof(AvbFaceDataNew);
  quint32 prevImage = 0;
...
bool Avatar::loadTorsoRecs(AvbStream *stream, bool oldTag, qint32 &adj) {
  ...
  const quint32 recSize =
      oldTag ? sizeof(AvbTorsoDataOld) : sizeof(AvbTorsoDataNew);
  quint32 prevImage = 0;
...
AvatarSimple::AvatarSimple() : Avatar(AvatarKind::Simple) {}

bool AvatarSimple::handleTag(AvbStream *stream, quint16 tag, quint16 size,
                             qint32 &adj) {
...
RenderedBody AvatarSimple::renderForEmotion(const Emotion &em) {
  ...
  for (int i = 0; i < bodies.size(); ++i) {
    ...
    if (bodies[index].emotion > 7.f && bodies[index].emotion < 100.f)
      continue;
    const double angle =
        qAbs(subtractAngles(bodies[index].emotion, em.emotion));
...
AvatarComplex::AvatarComplex() : Avatar(AvatarKind::Complex) {}

bool AvatarComplex::handleTag(AvbStream *stream, quint16 tag, quint16 size,
                              qint32 &adj) {
...
RenderedBody AvatarComplex::renderForEmotion(const Emotion &em) {
  ...
  for (int i = 0; i < torsos.size(); ++i) {
    ...
    if (torsos[index].emotion > 7.f && torsos[index].emotion < 100.f)
      continue;
    const double angle =
        qAbs(subtractAngles(torsos[index].emotion, em.emotion));
    if (angle < M_PI / NEMOTIONS || (torsos[index].emotion == emNeutral() &&
                                     torsos[index].intensity == 0.f)) {

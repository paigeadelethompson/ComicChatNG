...
SetupDialog::SetupDialog(AppSettings *settings, ArtManager *art,
                         QWidget *parent)
    : QDialog(parent), m_settings(settings), m_art(art) {
...
QWidget *SetupDialog::buildSettingsPage() {
  ...
  m_noComics =
      new QCheckBox(tr("Don't send Microsoft Chat specific information"));
...
QWidget *SetupDialog::buildCharacterPage() {
  ...
  for (int i = 0; i < m_avatarList->count(); ++i) {
    if (m_avatarList->item(i)->text().compare(m_settings->avatarName,
  ...
  connect(m_avatarList, &QListWidget::currentRowChanged, this,
          &SetupDialog::onAvatarRow);
  ...
  m_avatarPreview->setStyleSheet(
      QStringLiteral("QLabel { background: #fff; border: 1px solid #888; }"));
...
QWidget *SetupDialog::buildBackgroundPage() {
  ...
  for (int i = 0; i < m_backdropList->count(); ++i) {
    if (m_backdropList->item(i)->text().compare(m_settings->backdropName,
                                                Qt::CaseInsensitive) == 0) {
      row = i;
      break;
    }
  ...
  connect(m_backdropList, &QListWidget::currentRowChanged, this,
          &SetupDialog::onBackdropRow);

  m_backdropPreview = new QLabel;
  m_backdropPreview->setFixedSize(140, 100);
  m_backdropPreview->setAlignment(Qt::AlignCenter);
  m_backdropPreview->setStyleSheet(
      QStringLiteral("QLabel { background: #fff; border: 1px solid #888; }"));
...
void SetupDialog::refreshAvatarPreview() {
  const QString name = m_avatarList->currentItem()
                           ? m_avatarList->currentItem()->text()
                           : m_settings->avatarName;
  if (Avatar *av = m_art->avatar(name)) {
    const QImage img = av->iconImage();
    if (!img.isNull()) {
      m_avatarPreview->setPixmap(QPixmap::fromImage(
          img.scaled(96, 130, Qt::KeepAspectRatio, Qt::SmoothTransformation)));
      return;
    }
  }
...
void SetupDialog::refreshBackdropPreview() {
  const QString name = m_backdropList->currentItem()
                           ? m_backdropList->currentItem()->text()
                           : m_settings->backdropName;
  if (Backdrop *bd = m_art->backdrop(name)) {
    const QImage img = bd->image();
    if (!img.isNull()) {
      m_backdropPreview->setPixmap(QPixmap::fromImage(
          img.scaled(140, 100, Qt::KeepAspectRatio, Qt::SmoothTransformation)));
      return;
    }
  }
...
void SetupDialog::browseArtDir() {
  const QString dir = QFileDialog::getExistingDirectory(
      this, tr("Select ComicArt folder"), m_artDir->text());
  if (!dir.isEmpty())
...
void SetupDialog::accept() {
  ...
  m_settings->comicView =
      m_comicView ? m_comicView->isChecked() : m_settings->comicView;
  m_settings->save();

...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  if (!m_art.scan()) {
    statusBar()->showMessage(
        tr("Warning: no comic art found in %1").arg(m_settings.artDirectory));
  }
  ...
  connect(&m_irc, &IrcClient::connectionError, this,
          &MainWindow::onConnectionError);
  connect(&m_irc, &IrcClient::channelJoined, this,
          &MainWindow::onChannelJoined);
  connect(&m_irc, &IrcClient::channelParted, this,
  ...
  connect(&m_irc, &IrcClient::backdropAnnounce, this,
          &MainWindow::routeBackdrop);
  connect(&m_irc, &IrcClient::serverMessage, this,
          &MainWindow::routeServerMessage);
}
...
QMenu *MainWindow::buildFileMenu() {
  ...
  menu->addAction(tr("&New Connection...\tCtrl+N"), this,
                  &MainWindow::sessionConnect);
...
QMenu *MainWindow::buildFormatMenu() {
  ...
  menu->addAction(tr("&Fixed Pitch Font\tCtrl+F"), this,
                  &MainWindow::notImplemented);
...
QMenu *MainWindow::buildMemberMenu() {
  ...
  menu->addAction(tr("&Away from Keyboard..."), this,
                  &MainWindow::notImplemented);
  menu->addSeparator();
  ...
  menu->addAction(tr("Add to Notifi&cations..."), this,
                  &MainWindow::notImplemented);
...
QToolBar *MainWindow::buildMainToolbar() {
  ...
  bar->addAction(icons::main(0), tr("Connect"), this,
                 &MainWindow::sessionConnect)
      ->setToolTip(tr("Connect"));
  bar->addAction(icons::main(1), tr("Disconnect"), this,
                 &MainWindow::sessionDisconnect);
  bar->addAction(icons::main(2), tr("Enter Room"), this,
                 &MainWindow::enterRoom);
  bar->addAction(icons::main(3), tr("Leave Room"), this,
                 &MainWindow::leaveRoom);
  bar->addAction(icons::main(4), tr("Create Room"), this,
  ...
  bar->addSeparator();
  bar->addAction(icons::main(7), tr("Room List"), this,
                 &MainWindow::showRoomList);
  bar->addAction(icons::main(8), tr("User List"), this,
                 &MainWindow::notUserList);
  bar->addSeparator();
  bar->addAction(icons::main(9), tr("Open Favorites..."), this,
                 &MainWindow::notImplemented);
  return bar;
}
...
QToolBar *MainWindow::buildTextToolbar() {
  ...
  bar->setMovable(false);
  bar->setIconSize(QSize(16, 16));
  bar->addAction(icons::text(0), tr("Set Font..."), this,
                 &MainWindow::notImplemented);
  bar->addAction(icons::text(1), tr("Set Color..."), this,
                 &MainWindow::notImplemented);
...
void MainWindow::addToggleTextAction(QToolBar *bar, const QIcon &icon,
                                     const QString &text) {
...
QToolBar *MainWindow::buildUserToolbar() {
  ...
  bar->addAction(icons::user(1), tr("Get Identity"), this,
                 &MainWindow::notImplemented);
  bar->addAction(icons::user(2), tr("Ignore"), this,
                 &MainWindow::notImplemented);
  bar->addAction(icons::user(3), tr("Whisper Box"), this,
                 &MainWindow::notImplemented);
  bar->addSeparator();
  bar->addAction(icons::user(4), tr("Send E-mail"), this,
                 &MainWindow::notImplemented);
  bar->addAction(icons::user(5), tr("Visit Home Page"), this,
                 &MainWindow::notImplemented);
  bar->addAction(icons::user(6), tr("NetMeeting"), this,
                 &MainWindow::notImplemented);
  return bar;
}
...
void MainWindow::enterRoom() {
  ...
  m_irc.connectToHost(m_settings.server, m_settings.port, m_settings.nick,
                      m_settings.user, m_settings.realName);
...
void MainWindow::createRoom() { enterRoom(); }
...
void MainWindow::sessionDisconnect() { m_irc.disconnectFromHost(); }
...
void MainWindow::toggleTextView(bool on) { toggleComicView(!on); }
...
void MainWindow::about() {
  QMessageBox::about(
      ...
      tr("<h3>Chat: Microsoft Chat 2.5</h3>"
         ...
         "<p>Loads the original <code>.avb</code> / <code>.bgb</code> "
         "art.</p>"));
}
...
void MainWindow::onDisconnected() { m_status->setText(tr("Disconnected")); }
...
void MainWindow::routePrivmsg(const QString &channel, const QString &nick,
                              const QString &text) {
  if (RoomWidget *r = roomForChannel(channel, true))
    r->onPrivmsg(channel, nick, text);
}

void MainWindow::routeAction(const QString &channel, const QString &nick,
                             const QString &text) {
  if (RoomWidget *r = roomForChannel(channel, true))
...
void MainWindow::routeUserParted(const QString &channel, const QString &nick,
...
void MainWindow::routeNickChanged(const QString &oldNick,

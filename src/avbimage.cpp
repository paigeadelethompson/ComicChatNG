...
QImage imageFromIndexedBits(int width, int height, int bitCount,
                            const QVector<QRgb> &palette, const QByteArray &bits)
{
    ...
    auto sample = [&](int x, int yRow) -> int {
        const uchar *row = reinterpret_cast<const uchar *>(bits.constData()) + yRow * stride;
        if (bitCount == 1) {
...
bool getProperPalette(AvbStream *stream, const AvbImageRef &ref, AvbPalette *pal)
{
...
bool readZlibImage(AvbStream *stream, const AvbImageRef &ref, QImage *out)
{
    ...
    if (!stream->read32(&headerSize))
        return false;
    if (headerSize < sizeof(BmpInfoHeader) || headerSize > sizeof(BmpInfoHeader) * 6)
        return false;

    BmpInfoHeader bih {};
    bih.size = headerSize;
    const quint32 rest = headerSize - sizeof(quint32);
    if (stream->read(reinterpret_cast<char *>(&bih) + sizeof(quint32), rest) != rest)
        return false;
    ...
    const int expected = dibStorageWidth(bih.width, bih.bitCount) * qAbs(bih.height);
    if (bits.size() != expected)
        return false;

    *out = imageFromIndexedBits(bih.width, bih.height, bih.bitCount, pal.colors(), bits);
    return !out->isNull();
}
...
bool loadBmpFromStream(AvbStream *stream, QImage *out)
{
    ...
    if (stream->read(bits.data(), static_cast<quint32>(bitsSize)) != static_cast<quint32>(bitsSize))
        return false;
    ...
    if (bih.bitCount == 24 || bih.bitCount == 32) {
        ...
        // Fix header sizes relative to our slice — easier path: decode manually for 24bpp.
        ...
        for (int y = 0; y < absH; ++y) {
            ...
            const uchar *row = reinterpret_cast<const uchar *>(bits.constData()) + srcY * stride;
            QRgb *dst = reinterpret_cast<QRgb *>(img.scanLine(y));
    ...
    *out = imageFromIndexedBits(bih.width, bih.height, bih.bitCount, palette, bits);
    return !out->isNull();
}
...
QImage applyComicMask(const QImage &drawing, const QImage &mask)
{
    ...
    QImage m = mask.convertToFormat(QImage::Format_ARGB32).scaled(

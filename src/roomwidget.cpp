...
RoomWidget::RoomWidget(const QString &channel, ArtManager *art,
                       AppSettings *settings, IrcClient *irc, QWidget *parent)
    : QWidget(parent), m_channel(channel), m_art(art), m_settings(settings),
      m_irc(irc), m_roomBackdrop(settings->backdropName) {
  ...
  m_preview->setStyleSheet(
      QStringLiteral("QLabel { background: #ddd; border: 1px solid #888; }"));

  m_emotionPicker = new EmotionPicker;
  connect(m_emotionPicker, &EmotionPicker::emotionChanged, this,
          &RoomWidget::onEmotionChanged);
  connect(this, &RoomWidget::clearHistoryRequested, this,
          [this] { m_textView->clear(); });
  ...
  connect(modeGroup, &QButtonGroup::idClicked, this,
          [this](int id) { m_sayMode = id; });
  ...
  if (text.startsWith(QLatin1Char('/'))) {
    ...
    if (m_sayMode == 3) {
      ...
      if (!m_whisperTarget.isEmpty()) {
        ...
        appendText(
            QStringLiteral("[whisper to %1] %2").arg(m_whisperTarget, text));
      } else {
        appendText(tr("[whisper] Right-click a member and choose “Whisper…” to "
                      "pick a target."));
      }
    } else {
      if (!localOnly)
...
void RoomWidget::addComicLine(const QString &nick, const QString &text,
                              bool isAction) {
  addComicBalloon(nick,
                  isAction ? QStringLiteral("* %1 %2").arg(nick, text) : text,
...
void RoomWidget::addComicBalloon(const QString &nick, const QString &text,
  ...
  if (m_pageView->panelCount() > 0) {
    ...
    if (last.balloons.size() < kMaxBalloonsPerPanel &&
        !last.containsSpeaker(nick) && kind != BalloonKind::Action) {
      panel = last;
      merging = true;
    }
  }
  ...
  if (Avatar *av = m_art->avatar(name)) {
    ...
    if (!img.isNull())
      m_preview->setPixmap(QPixmap::fromImage(
          img.scaled(96, 120, Qt::KeepAspectRatio, Qt::SmoothTransformation)));
  }
}
...
void RoomWidget::showRoomMenu(const QPoint &gpos) {
  ...
  connect(menu.addAction(tr("Clear &History")), &QAction::triggered, this,
          [this] {
            if (m_textView)
              m_textView->clear();
            m_pageView->clear();
...
void RoomWidget::showMemberMenu(const QPoint &pos) {
  ...
  connect(menu.addAction(tr("Send &File...")), &QAction::triggered, this,
          &RoomWidget::notImplemented);
  connect(menu.addAction(tr("Visit H&ome Page")), &QAction::triggered, this,
          &RoomWidget::notImplemented);
  connect(menu.addAction(tr("Net&Meeting")), &QAction::triggered, this,
          &RoomWidget::notImplemented);
  menu.addSeparator();
  connect(menu.addAction(tr("&Version")), &QAction::triggered, this,
          &RoomWidget::notImplemented);
  connect(menu.addAction(tr("Lag &Time")), &QAction::triggered, this,
          &RoomWidget::notImplemented);
...
void RoomWidget::showRoomProperties() {
  ...
  if (m_irc && m_irc->isConnected())
    m_irc->sendRaw(
        QStringLiteral("TOPIC %1 :%2").arg(m_channel, topic->text()));
...
void RoomWidget::onWhisper(const QString &sender, const QString &who,
                           const QString &text) {
...
void RoomWidget::onUserJoined(const QString &channel, const QString &nick) {
  if (channel.compare(m_channel, Qt::CaseInsensitive) != 0)
    return;
  ensureMember(nick);
  if (nick.compare(m_settings->nick, Qt::CaseInsensitive) == 0 && m_irc &&
      m_irc->isConnected()) {
...
void RoomWidget::onUserParted(const QString &channel, const QString &nick,
...
void RoomWidget::onNickChanged(const QString &oldNick, const QString &newNick) {
  ...
  if (m_userAvatars.contains(oldNick.toLower())) {
    m_userAvatars.insert(newNick.toLower(),
                         m_userAvatars.take(oldNick.toLower()));
  }

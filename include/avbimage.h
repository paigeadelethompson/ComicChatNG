...
// Apply a 1-bpp or grayscale mask so white=transparent, black=opaque (Comic
// Chat GDI style).
QImage applyComicMask(const QImage &drawing, const QImage &mask);

// Compute whether a pixel's luma marks it as "opaque" (black drawing) per Comic
// Chat masks.
...
QImage dibToQImage(const QByteArray &bmiHeaderPlusColors,
                   const QByteArray &bits, int width, int height, int bitCount,
